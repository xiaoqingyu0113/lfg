...
import numba
import torch
from lfg.model_traj.mnnl import MNNL
import gtsam
...
mnn = MNNL(z0=0.010)
mnn.load_state_dict(torch.load(WEIGHTS_DIR + '/model_MNNL.pth'))
...
# for k in global_bc_params.keys():
#     print(k)
...
def _bounce_forward(recode_weight, recode_bias, layer1_weight, layer1_bias,
    ...
    h0_ = x @ layer1_weight.T + layer1_bias
    h0 = np.maximum(h0_, alpha*h0_)
    h1_ = h0 @ layer2_weight.T + layer2_bias
    h1 = np.maximum(h1_, alpha*h1_) * h0 + h0

    y1 = h1 @ dec_0_weight.T + dec_0_bias

    v2d_local_new = R2d @ y1[:2] * 3.0
    vz_new = y1[2:3] * 3.0
    w2d_local_new = R2d @ y1[3:5] * 7.0
    wz_new = y1[5:6] * 7.0
...
def _bounce_jacobian(recode_weight, recode_bias, layer1_weight, layer1_bias,
    ...
    # layer 1
    alpha = DTYPE(0.01)
    h0 = x @ layer1_weight.T + layer1_bias
    J_h0_v = layer1_weight @ J_x_v
    J_h0_w = layer1_weight @ J_x_w


    h0m = np.maximum(alpha*h0, h0)
    tmp = np.ones_like(h0, dtype=DTYPE)
    tmp[h0 < alpha*h0] = alpha
    ...
    h1m = np.maximum(h1, alpha*h1)
    tmp = np.ones_like(h1, dtype=DTYPE)
    tmp[h1 < alpha*h1] = alpha
    J_h1m_ = np.diag(tmp) 
    ...
    # decode layer
    y1 = h1mul @ dec_0_weight.T + dec_0_bias
    J_y1_v = dec_0_weight @ J_h1mul_v
    J_y1_w = dec_0_weight @ J_h1mul_w
    

    # --------------------------------- above passed accuracy test ------------------------------
                                
    J_vnew_v = np.array([
                        [J_R2d_v2d[0,0] * y1[0] + R2d[0,0] * J_y1_v[0, 0] + J_R2d_v2d[1,0] * y1[1] + R2d[0,1] * J_y1_v[1, 0],
                        J_R2d_v2d[0,1] * y1[0] + R2d[0,0] * J_y1_v[0, 1] + J_R2d_v2d[1,1] * y1[1] + R2d[0,1] * J_y1_v[1, 1],
                        R2d[0,0] * J_y1_v[0, 2] + R2d[0,1] * J_y1_v[1, 2]],

                        [J_R2d_v2d[2,0] * y1[0] + R2d[1,0] * J_y1_v[0, 0] + J_R2d_v2d[3,0] * y1[1] + R2d[1,1] * J_y1_v[1, 0],
                        J_R2d_v2d[2,1] * y1[0] + R2d[1,0] * J_y1_v[0, 1] + J_R2d_v2d[3,1] * y1[1] + R2d[1,1] * J_y1_v[1, 1],
                        R2d[1,0] * J_y1_v[0, 2] + R2d[1,1] * J_y1_v[1, 2]],
    ...
    J_vnew_w = np.array([R2d[0,0] * J_y1_w[0,0] + R2d[0,1] * J_y1_w[1,0],
                        R2d[0,0] * J_y1_w[0,1] + R2d[0,1] * J_y1_w[1,1],
                        R2d[0,0] * J_y1_w[0,2] + R2d[0,1] * J_y1_w[1,2],
                        R2d[1,0] * J_y1_w[0,0] + R2d[1,1] * J_y1_w[1,0],
                        R2d[1,0] * J_y1_w[0,1] + R2d[1,1] * J_y1_w[1,1],
                        R2d[1,0] * J_y1_w[0,2] + R2d[1,1] * J_y1_w[1,2],
                        J_y1_w[2,0],
                        J_y1_w[2,1],
                        J_y1_w[2,2]]).reshape(3,3) * DTYPE(3.0)

    J_wnew_v = np.array([J_R2d_v2d[0,0] * y1[3] + R2d[0,0] * J_y1_v[3,0] + J_R2d_v2d[1,0] * y1[4] + R2d[0,1] * J_y1_v[4,0], #bad
                        J_R2d_v2d[0,1] * y1[3] + R2d[0,0] * J_y1_v[3,1] + J_R2d_v2d[1,1] * y1[4] + R2d[0,1] * J_y1_v[4,1], # bad
                        R2d[0,0] * J_y1_v[3,2] + R2d[0,1] * J_y1_v[4,2], #good          
                        J_R2d_v2d[2,0] * y1[3] + R2d[1,0] * J_y1_v[3,0] + J_R2d_v2d[3,0] * y1[4] + R2d[1,1] * J_y1_v[4,0], #bad
                        J_R2d_v2d[2,1] * y1[3] + R2d[1,0] * J_y1_v[3,1] + J_R2d_v2d[3,1] * y1[4] + R2d[1,1] * J_y1_v[4,1], #bad
                        R2d[1,0] * J_y1_v[3,2] + R2d[1,1] * J_y1_v[4,2], #good
                        J_y1_v[5,0], #bad
                        J_y1_v[5,1], #bad
                        J_y1_v[5,2]]).reshape(3,3) * DTYPE(7.0) #good 
    
    J_wnew_w = np.array([R2d[0,0] * J_y1_w[3,0] + R2d[0,1] * J_y1_w[4,0],
                        R2d[0,0] * J_y1_w[3,1] + R2d[0,1] * J_y1_w[4,1],
                        R2d[0,0] * J_y1_w[3,2] + R2d[0,1] * J_y1_w[4,2],
                        R2d[1,0] * J_y1_w[3,0] + R2d[1,1] * J_y1_w[4,0],
                        R2d[1,0] * J_y1_w[3,1] + R2d[1,1] * J_y1_w[4,1],
                        R2d[1,0] * J_y1_w[3,2] + R2d[1,1] * J_y1_w[4,2],
                        J_y1_w[5,0],
                        J_y1_w[5,1],
                        J_y1_w[5,2]]).reshape(3,3) * DTYPE(7.0)
...
def _aero_forward(recode_weight, recode_bias, layer1_weight, layer1_bias, 
                        layer2_weight, layer2_bias, 
                        dec_0_weight, dec_0_bias, bias, v, w):
    w = w @ recode_weight.T + recode_bias
    R, v_local, w_local = gs(v, w)     
    
    feat = np.array([v_local[0], w_local[0], w_local[1]])
    alpha = 0.01
    h_ = feat @ layer1_weight.T + layer1_bias
    h = np.maximum(h_, alpha*h_) # leaky relu
    h2_ = h @ layer2_weight.T + layer2_bias
    h2 = np.maximum(h2_, alpha*h2_)*h + h # leaky relu
    y = h2 @ dec_0_weight.T + dec_0_bias
...
def _aero_jacobian(recode_weight, recode_bias, layer1_weight, layer1_bias, 
    ...
    h2m = np.maximum(h2, alpha*h2)
    tmp = np.ones_like(h2, dtype=DTYPE)
    tmp[h2 < alpha*h2] = alpha
    ...
    J_y1 = dec_0_weight @ J_h2mul # 6x32

    # y1m = np.maximum(y1, alpha*y1)
    # tmp = np.ones_like(y1, dtype=DTYPE)
    # tmp[y1 < alpha*y1] = alpha
    ...
    J_r = J_rvw[:9,:]
    J_y3_y1 = R # 3x3
    J_y3_r = np.stack((np.concatenate((y1, np.zeros(3,dtype=DTYPE), np.zeros(3,dtype=DTYPE))), 
                        np.concatenate((np.zeros(3,dtype=DTYPE), y1, np.zeros(3,dtype=DTYPE))),
                        np.concatenate((np.zeros(3,dtype=DTYPE), np.zeros(3,dtype=DTYPE), y1)),)) # 3x9
    J_y3 = J_y3_r @ J_r + J_y3_y1 @ J_y1
