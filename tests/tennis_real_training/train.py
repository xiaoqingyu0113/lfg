...
class RealTrajectoryDataset(Dataset):
    def __init__(self, data_folder, interpolate=300):
        self.interpolate = interpolate
        data_np = self.read_all_txt_from_folder(Path(data_folder))
    ...
    def read_all_txt_from_folder(self, folder) -> np.ndarray:
        cvs_files = list(folder.glob('*.txt'))
        for f in cvs_files:
            data_tmp = self.preprocess_data(self.read_from_txt(f))
            if 'data' in locals():
...
def train_loop(cfg):
    ...
    for epoch in range(cfg.model.num_epochs):
        for i, data in enumerate(train_loader):
            ...
            # zero out infinite gradients
            for name, param in model.named_parameters():
                if param.grad is not None:
                    # Check for infinite or NaN values in gradients
                    inf_or_nan_mask = torch.isinf(param.grad) | torch.isnan(param.grad)
                    if inf_or_nan_mask.any():
