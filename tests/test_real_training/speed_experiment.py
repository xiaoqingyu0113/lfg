...
class TestCases:
    ...
    def test_aero_forward():
        ...
        v = v.reshape(-1).cpu().detach().numpy().astype(np.float32)
        w =  w.reshape(-1).cpu().detach().numpy().astype(np.float32)
        ...
        y_global = aero_forward(np.random.rand(3).astype(np.float32), np.random.rand(3).astype(np.float32))
